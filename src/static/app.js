    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = details.participants.length
          ? details.participants.map(email => `<li><span class="participant-email">${email}</span> <button class="remove-btn" data-activity="${name}" data-email="${email}" aria-label="Remove ${email}">🗑️</button></li>`).join("")
  ...
  signupForm.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        messageDiv.className = "success";
        signupForm.reset();
        // Refresh the activities list and dropdown so the new participant appears
        fetchActivities();
      } else {
